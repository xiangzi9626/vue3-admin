...
const addUser = async (userInfo: any) => {
  ...
  const res: any = await request.post("/admin/add_user", userInfo)
  if (res.code == 200) {
...
const getData = async (currentPage: number, pageSize: number) => {
  const res: any = await request.post("/admin/adminlist", {currentPage: currentPage, pageSize: pageSize})
  data.list = res.data
...
const editUserInfo = async (userInfo: any) => {
  const res: any = await request.post("/admin/edit_user", userInfo)
  if (res.code == 200) {
    ElSuccess("提交成功");
...
const modifyPassword = () => {
  request.post("/admin/modify_user_password", form).then((res: any) => {
    if (res.code == 200) {
      ElSuccess("用户密码修改成功")

  ...
  >
    ...
    <el-form-item label="商品主图" prop="img">
      <el-upload
          ...
          class="upload-demo"
          :action="CONFIG.SERVER_URL+'/admin/upload_goods_img'"
          name="file"
    ...
    <el-form-item label="分类" prop="cid">
      <el-tree-select v-model="ruleForm.cid" placeholder="请选择" :data="data.goodsCategory" :default-expand-all="true"
                      :check-strictly="true" v-if="data.goodsCategory.length>0"/>
    </el-form-item>
...
const getGoodsById = async () => {
  ...
  if (id === undefined || !(/^\d+$/.test(id))) {
    router.push("/admin/goods/goodslist")
    return;
  }
  const res: any = await request.post("/admin/get_goods_by_id?id=" + id)
  ruleForm.id = res.id;
...
const getData = async () => {
  const res: any = await request.post("/admin/goods_category")
...
const handleRemove: UploadProps['onRemove'] = (uploadFile, uploadFiles) => {
  request.post("/admin/delete_goods_img", {fileName: ruleForm.img}).then((res: any) => {
    ruleForm.img = ""
  })
}
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      request.post("/admin/update_goods", ruleForm).then((res: any) => {
        if (res.code == 200) {
          ElSuccess("更新成功")
          router.push("/admin/goods/goodslist")
        } else {

<template>
  ...
  >
    ...
    <el-form-item label="上级菜单" prop="pid">
      <el-tree-select v-model="ruleForm.pid" :data="data" :default-expand-all="true" :check-strictly="true" placeholder="请选择"/>
    </el-form-item>
...
const getMenuById=async ()=>{
  const id:any=useRoute().query.id
   if (id===undefined || !(/^\d+$/.test(id))){
    router.push("/admin/menu/menulist")
    return;
  }
  const res:any =await request.post("/admin/get_menu_by_id?id="+id)
  ruleForm.id=res.id;
...
const data=reactive(buildRoute(menuStore().menus))
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      request.post("/admin/update_menu",ruleForm).then((res:any)=>{
        if(res.code==200){
          ElSuccess("添加成功")
          request.post("/admin/menu").then((res:any)=>{
            menuStore().menus=res
            router.push("/admin/menu/menulist")
          })
        }else{

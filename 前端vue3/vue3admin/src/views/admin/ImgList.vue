...
const getImg = async () => {
  const res: any = await request.post("/admin/file_list?currentPage=" + data.currentPage + "&pageSize=" + data.pageSize)
...
const del = async () => {
  ...
  const res: any = await request.post("/admin/batch_delete_img", arr)
  if (res.code == 200) {
    ElSuccess("删除成功")

...
const getData = async () => {
  const res = await request.post("/admin/goods_category")
  //@ts-ignore
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      request.post("/admin/add_goods_category", ruleForm).then((res: any) => {
        if (res.code == 200) {
...
const handleDelete = (item: any) => {
  request.post("/admin/delete_goods_category", {id: item.categoryId}).then((res: any) => {
    if (res.code == 200) {
      ElSuccess(res.msg)
      getData()
    } else {
